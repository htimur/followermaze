...
object Parser {
  ...
  object StatusUpdatedEvent extends EventParser[StatusUpdated] {
    override val pattern = "^([0-9]+)\\|S\\|([0-9]+)$".r

    override def unapply(arg: String): Option[StatusUpdated] = pattern.unapplySeq(arg) match {
      case Some(sec :: from :: Nil) =>
        Some(StatusUpdated(sec.toInt, from.toInt))
      case _ => None
    }
  }
